...
/* Helper methods */
struct MONRES getScreenResolution() {
    ...
    struct MONRES res = {.X = target.rcMonitor.right, .Y = target.rcMonitor.bottom};
    return res;
}
...
void drawStatus(enum STATUS status, HWND hWnd, struct INIPARAM iniParam)
{
    ...
    // set window to top
    struct MONRES res = getScreenResolution();
    SetWindowPos(hWnd, HWND_TOPMOST, res.X - WND_WIDTH, res.Y - WND_HEIGHT + 10, WND_WIDTH, WND_HEIGHT, SWP_NOMOVE | SWP_NOSIZE);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
    ...
    hWnd = CreateWindowEx(
        ...
        lpszTitle,
        WS_POPUP,
        res.X - WND_WIDTH, res.Y - WND_HEIGHT + 10,
